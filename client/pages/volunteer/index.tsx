...
const VolunteerChart = () => {
  const { data: dataApplyActivity, refetch } = useQuery(
    ['listApplyActivityByVolunteer'],
    () => volunteerService.getAppliedActivities()
  );

  const appliedVolunteers =
    dataApplyActivity?.data.data.appliedVolunteers || [];
      ...
      tooltip: {
        callbacks: {
          label: function (context: any) {
            ...
            return `${label}: ${value} - ${getTextForStatus(
              context.dataIndex
            )}`;
          }
        }
...
VolunteerChart.getLayout = (children: React.ReactNode) => (

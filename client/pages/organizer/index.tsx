...
const OrganizerChart = () => {
  ...
  const { data: dataRequestVolunteers } = useQuery(
    ...
    datasets: [
      {
        label: 'Thống kê',
        data: [
          dataVolunteer?.data.data.length,
          dataActivity?.length,
          dataRequestVolunteers?.data.data.requestVolunteers
        ],
...
OrganizerChart.getLayout = (children: React.ReactNode) => (
